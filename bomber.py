...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/jagadish-pattanaik/Bomb/archive/dev.zip"
        dir_name = "Bomb-dev"
    else:
        zip_url = "https://github.com/jagadish-pattanaik/Bomb/archive/master.zip"
        dir_name = "Bomb-master"
    ...
    if success:
        ...
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/jagadish-pattanaik/Bomb.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/jagadish-pattanaik/Bomb.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/jagadish-pattanaik/Bomb/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("Bomb is up-to-date")
...
def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/jagadish-pattanaik/Bomb/raw/dev/.notify"
        else:
            url = "https://github.com/jagadish-pattanaik/Bomb/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("Bomb was created by jagadish-pattanaik")
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 9999999999, "call": 9999999999, "mail": 9999999999}
        cc, target = "", ""
        if mode in ["sms", "call"]:
            cc, target = get_phone_info()
            if cc != "91":
                max_limit.update({"sms": 9999999999})
        elif mode == "mail":
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("Bomb will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['jagadish-pattanaik']
...
description = """Bomb - Your Friendly Spammer Application

Bomb can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

Bomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by jagadish-pattanaik !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start Bomb with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start Bomb with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start Bomb with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update Bomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current Bomb contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current Bomb version")
